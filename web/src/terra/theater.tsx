import { useEffect, useRef, useState, type ReactNode, type RefObject, type PointerEvent as ReactPointerEvent } from "react";
...
function selectionLabel(sel: LiveSelection): string {
  return sel.name ?? sel.tag ?? "element";
}

function liveSelectionId(sel: LiveSelection): string {
        ...
        {tab === "demo" ? (
          live ? (
            <div className="sh-replica sh-replica--live" data-node={node.id}>
              <LiveFrame picking={picking} frameRef={frameRef} />
            </div>
          ) : (
            <div
