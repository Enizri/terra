...
func (r *hostRunner) Start(repoURL string) (string, error) {
	...
	proxyURL, err := serveProxy(key, targetBase, authFix)
	if err != nil {
		stop(cmd)
		stop(apiCmd)
		return "", err
...
func serveProxy(repoKey, targetBase string, authFix func(*http.Request)) (string, error) {
...
// auth headers (see seedDemoAuth). publicPrefix is the browser-visible path
// prefix for the script tag (e.g. "/__live/id"); the handler itself still
// serves "/__terra/select.js" after the live hub strips the prefix.
