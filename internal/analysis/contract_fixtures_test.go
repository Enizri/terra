...
// TestContractFixtures ensures contracts/fixtures/analysis-map.v1.json decodes
// with the same strict rules as the golden case study.
func TestContractFixtures(t *testing.T) {
	path := filepath.Join("..", "..", "contracts", "fixtures", "analysis-map.v1.json")
	...
	goldenPath := filepath.Join("..", "..", "case-studies", "memos.map.json")
	...
	if string(fb) != string(gb) {
		t.Fatal("contracts/fixtures/analysis-map.v1.json drifts from case-studies/memos.map.json")
	}
}
