...
COPY backend/local-llm/pyproject.toml ./backend/local-llm/
COPY backend/local-llm/terra_local_llm/ ./backend/local-llm/terra_local_llm/
...
RUN pip install --no-cache-dir './backend/local-llm[local]'
